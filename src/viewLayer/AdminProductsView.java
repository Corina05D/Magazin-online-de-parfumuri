...
public class AdminProductsView extends JFrame {
    ...
    public AdminProductsView(String[][] rowData) {
        ...
        String[] columnNames = new String[]{"Title", "Rating", "Volume", "Fragrance", "Intensity", "Alcohol", "Price"};
    ...
    public MenuItem getEditedMenuItem() {
        ...
        int newVolume = Integer.parseInt((String) table.getValueAt(selectedRow, 2));
        int newFragrance = Integer.parseInt((String) table.getValueAt(selectedRow, 3));
        int newIntensity = Integer.parseInt((String) table.getValueAt(selectedRow, 4));
        int newAlcohol = Integer.parseInt((String) table.getValueAt(selectedRow, 5));
        ...
        for (MenuItem menuItem : DeliveryService.menuItems) {
            if (menuItem.getTitle().equals(getSelectedTitle())) {
                if (menuItem instanceof BaseProduct) {
                    return new BaseProduct(menuItem.getTitle(), newRating, newVolume, newFragrance, newIntensity, newAlcohol, newPrice);
                }
                if (menuItem instanceof CompositeProduct) {
                    return new CompositeProduct(menuItem.getTitle(), newRating, newVolume, newFragrance, newIntensity, newAlcohol, newPrice);
                }
            }
        }
        return null;
